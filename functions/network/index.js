...
async function network(ctx) {
    ...
    try {
        ...
        const edgeTypes = new Map(); // "src|tgt" -> relation type
        for (const doc of rawEdges) {
            ...
            adj.get(tgt).add(src);
            if (doc.SourceLabel)
                nameByKey.set(src, doc.SourceLabel);
            if (doc.TargetLabel)
                nameByKey.set(tgt, doc.TargetLabel);
        ...
        try {
            ...
            const attrRows = (await zcql.executeZCQLQuery(attrQ).catch(() => [])) || [];
            ...
            for (const r of attrRows) {
                ...
                if (row.PersonName)
                    nameByKey.set(pk, row.PersonName);
                if (row.PhoneNumber) {
                    if (!byPhone.has(row.PhoneNumber))
                        byPhone.set(row.PhoneNumber, []);
                    byPhone.get(row.PhoneNumber).push(pk);
                }
                if (row.VehicleReg) {
                    if (!byVehicle.has(row.VehicleReg))
                        byVehicle.set(row.VehicleReg, []);
            ...
            for (const [, keys] of byPhone)
                for (let i = 0; i < keys.length; i++)
                    for (let j = i + 1; j < keys.length; j++)
                        addSharedEdge(keys[i], keys[j], 'PHONE');
            for (const [, keys] of byVehicle)
                for (let i = 0; i < keys.length; i++)
                    for (let j = i + 1; j < keys.length; j++)
                        addSharedEdge(keys[i], keys[j], 'VEHICLE');
            for (const [, keys] of byAddress)
                for (let i = 0; i < keys.length; i++)
                    for (let j = i + 1; j < keys.length; j++)
                        addSharedEdge(keys[i], keys[j], 'ADDRESS');
        }
        catch (err) {
            ...
            logger_1.logger.warn('network.shared-attr-skip', { requestId, error: String(err) });
        }
        ...
        while (frontier.length) {
            ...
            if (depth < hops) {
                for (const n of adj.get(key) ?? []) {
                    if (!visited.has(n))
                        frontier.push({ key: n, depth: depth + 1 });
                }
            }
        }
        ...
        logger_1.logger.info('network.query', {
            requestId,
            route: '/network',
            seed: seed ?? 'all',
            hops,
        ...
        if (url.pathname.endsWith('/communities')) {
            ...
            logger_1.logger.info('network.communities', { requestId, count: communities.length });
            return (0, errors_1.ok)(communities);
        }
 ...
 */
function detectCommunities(adj, nameByKey) {
